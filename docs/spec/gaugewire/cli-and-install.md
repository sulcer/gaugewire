...
auth row, which reads `key valid; <warning>`. Each row fails independently: a key that cannot
be resolved fails all three with the key error, since none of them can be answered without a client; an unconfigured
