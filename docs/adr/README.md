...
|---|---|---|---|---|
| 2026-09-20 | [only-a-recent-payload-may-move-a-window](2026-09-20-only-a-recent-payload-may-move-a-window.md) | A reading whose reset has passed is refused; only a payload dated by its own five-hour window may say which window is open; within one window the highest usage wins, whatever the payload's age | accepted | Amends claude-statusline-is-the-only-quota-source (the staleness guard) |
